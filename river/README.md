...
local function run_autostarts(autostart_commands)
    for _, cmdline in ipairs(autostart_commands) do
        exec(fmt([[riverctl spawn '%s']], cmd(cmdline)))  -- 问题在这里！
    end
end
```


那么会产生另一个问题：riverctl spawn 会创建一个新的子进程来执行命令，这个子进程中的环境变量设置不会影响父进程（也就是您的 riverwm 会话）。


## 二、解决办法：使用环境包装器启动river

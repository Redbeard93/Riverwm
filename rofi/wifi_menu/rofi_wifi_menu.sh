...
YOFF=17
XOFF=0
FONT="Fira Code Nerd Font Mono 9"

if [ -r "$DIR/.config/rofi/wifi" ]; then
	source "$DIR/config/rofi/wifi"
elif [ -r "$HOME/.config/rofi/wifi_menu/wifi" ]; then
	source "$HOME/.config/rofi/wifi_menu/wifi"
else
...
if [ "$CHENTRY" = "manual" ] ; then
	...
	if [[ $(echo "$KNOWNCON" | grep "$CHSSID") = "$CHSSID" ]]; then
		...
		if [[ "$CHENTRY" =~ "WPA2" ]] || [[ "$CHENTRY" =~ "WEP" ]]; then
			WIFIPASS=$(echo "please input password, if connection is stored, hit enter" | rofi -dmenu -p "password: " -lines 1 -font "$FONT" )
		fi
